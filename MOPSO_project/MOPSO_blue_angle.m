...
function delta_velocity = MOPSO_blue_angle(time)
global decide_time_blue;
decide_time_blue = time;
% 设置参数
...
time = decide_time_blue;
endtime = Initialtime + seconds (decide_time_blue);

try
    blue_rv_2 = twoBodyOrbitRV(blue_rv,time);
    Red_rv_2 = twoBodyOrbitRV(red_rv,time);
    endtime = datetimeToVector(endtime);
    [idealDegree,actualDegree] = IlluminationAngle(blue_rv_2,Red_rv_2,endtime);
    D = actualDegree;
    value =D;
catch
    value=+inf;
