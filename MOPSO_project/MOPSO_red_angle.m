...
function delta_velocity = MOPSO_red_angle(time)
...
endtime = Initialtime + seconds (decide_time_red);

try
    blue_rv_2 = twoBodyOrbitRV(blue_rv,time);
    Red_rv_2 = twoBodyOrbitRV(red_rv,time);
    endtime = datetimeToVector(endtime);
    [idealDegree,actualDegree] = IlluminationAngle(Red_rv_2,blue_rv_2,endtime);
    D = actualDegree;
    value =D;
catch
    value=+inf;
